...
- popup の Daily チェックをオンにすると、ローカル日付の `Daily yyyy-MM-dd` ノートブックにも現在ページの URL を追加します。
- Daily ノートブックは同名があれば再利用し、なければ NotebookLM に新規作成します。Daily チェック状態は前回値を記録し、未記録時はオフです。
...
- 週次/月次プリセット、命名テンプレート編集、URLパターンマッチング、Deep Dive 自動生成は行いません。
